...
class CostExplorer:
    ...
    def generateExcel(self):
        ...
        reportName = "cost_explorer_report_" + accountID + "_" + datetime.date.today().strftime("%Y-%m-%d") + ".xlsx"
        os.chdir('/tmp')
        ...
        if os.environ.get('S3_BUCKET'):
            s3 = boto3.client('s3')            
            s3.upload_file(reportName, os.environ.get('S3_BUCKET'), reportName)
        if os.environ.get('SES_SEND'):
            ...
            text = "Please find your monthly Cost Explorer report attached\n\n"
            msg.attach(MIMEText(text))
            with open(reportName, "rb") as fil:
                part = MIMEApplication(
                    fil.read(),
                    Name=reportName
                )
            part['Content-Disposition'] = 'attachment; filename="%s"' % reportName
            msg.attach(part)
            #SES Sending
